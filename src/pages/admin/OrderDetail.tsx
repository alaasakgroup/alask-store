...
export default function AdminOrderDetailPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      await api.updateOrder(order.id, { status, adminNote });
      toast.success('تم حفظ التغييرات');
      setOrder({ ...order, status, adminNote });
    } catch (error) {
          ...
          <div className="bg-white rounded-lg border p-8 print:border-0 print:shadow-none" id="invoice">
            ...
            <div className="flex items-start justify-between mb-8 pb-6 border-b">
              <div className="flex items-center gap-3">
                {settings.logoUrl && (
                  <div className={`overflow-hidden ${settings.logoShape === 'circle' ? 'rounded-full' : 'rounded-lg'}`}>
                    <img 
